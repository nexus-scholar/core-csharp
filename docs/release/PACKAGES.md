...
and later accepted package-boundary ADRs. It contains nineteen domain and
evidence-contract libraries. `NexusScholar.CorpusSnapshots` and
`NexusScholar.Screening.CorpusSnapshots` and `NexusScholar.Reporting` were added
by ADR 0033 so downstream consumers can restore the immutable corpus authority,
its snapshot-to-Screening bridge, and deterministic review projections. CLI, workspace orchestration, application
...
6. loads all nineteen expected assemblies;
